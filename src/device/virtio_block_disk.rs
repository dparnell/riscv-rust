...
impl VirtioBlockDisk {
	...
	fn transfer_from_disk(&mut self, memory: &mut MemoryWrapper, mem_address: u64, disk_address: u64, length: u64) {
		...
		for i in 0..(length / 8) {
			...
			memory.store_u64(mem_address + i * 8, self.contents[disk_index]);
		}
	}
	...
	fn transfer_to_disk(&mut self, memory: &mut MemoryWrapper, mem_address: u64, disk_address: u64, length: u64) {
		...
		for i in 0..(length / 8) {
			let disk_index = ((disk_address + i * 8) >> 3) as usize;
			self.contents[disk_index] = memory.load_u64(mem_address + i * 8);
		}
	}
	...
	fn handle_disk_access(&mut self, memory: &mut MemoryWrapper) {
		...
		let _avail_flag = memory.load_u16(base_avail_address) as u64;
		let _avail_index = memory.load_u16(base_avail_address.wrapping_add(2)) as u64;
		let desc_index_address = base_avail_address.wrapping_add(4).wrapping_add((self.used_ring_index as u64 % queue_size) * 2);
		let desc_head_index = (memory.load_u16(desc_index_address) as u64) % queue_size;
		...
		*/
		...
		loop {
			...
			let desc_addr = memory.load_u64(desc_element_address);
			let desc_len = memory.load_u32(desc_element_address.wrapping_add(8));
			let desc_flags = memory.load_u16(desc_element_address.wrapping_add(12));
			desc_next = (memory.load_u16(desc_element_address.wrapping_add(14)) as u64) % queue_size;
			...
			*/
			...
			match desc_num {
				0 => {
					...
					_blk_type = memory.load_u32(desc_addr);
					_blk_reserved = memory.load_u32(desc_addr.wrapping_add(4));
					blk_sector = memory.load_u64(desc_addr.wrapping_add(8));
					/*
						...
						true => { // write to disk
							if (desc_addr % 8) == 0 && ((blk_sector * SECTOR_SIZE) % 8) == 0 &&
								...
								for i in 0..desc_len as u64 {
									let data = memory.load_u8(desc_addr + i);
						...
						false => { // read from disk
							if (desc_addr % 8) == 0 && ((blk_sector * SECTOR_SIZE) % 8) == 0 &&
								...
								for i in 0..desc_len as u64 {
									...
									memory.store_u8(desc_addr + i, data);
								}
							}
						}
					};
				...
				2 => {
					...
					memory.store_u8(desc_addr, 0); // 0 means succeeded
				},
				_ => {}
			};
		...
		memory.store_u32(base_used_address.wrapping_add(4).wrapping_add((self.used_ring_index as u64 % queue_size) * 8), desc_head_index as u32);

		self.used_ring_index = self.used_ring_index.wrapping_add(1);
		memory.store_u16(base_used_address.wrapping_add(2), self.used_ring_index);
	}
}

impl Device for VirtioBlockDisk {
